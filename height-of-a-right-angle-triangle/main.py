from manim import *
import math # just for the  atan2()  funcation

# I started writign comments, but then got lazy.
# If you want to, you can ask me an I will write comments for you.
# I mean, it did take me about 35 hours to figure this stuff out
# myself, and I am sure most of the stuff isn't even done in the
# best way possible.
# Kindest regards - Dimitri.
...
class PythagorasHeight(Scene):
	...
	def build_triangle(self):
		...
		# Constructing lines of the triangle
		...
		# Add the right angle marker to the triangle
		self.triangle += ra

		# Show the creation of the right angle marker
		self.play(Create(ra))

		self.wait(0.5)

		# Rotate the triangle so that it lies on it's hyoptenuse
		self.play(self.triangle.animate.rotate(math.atan2(4,3) + PI / 2))
		...
		# Right angle marker at the perpendicular
		p_ra = RightAngle(p_line, self.triangle[2], quadrant=[-1,-1], length=0.3, stroke_width=2)

		# The VGroup representing the perpendicular
		p = VGroup(p_line, p_ra)

		# Show the creation of the perpendicular
		self.play(
			Create(p_line, run_time=0.4),
			Create(p_ra, run_time=0.4)
		)

		# Add the perpendicular to the right triangle
		self.triangle += p
	...
	def add_side_lengths(self):
		# the 
		self.height = MathTex("h", color=self.colors[3]).move_to(self.triangle[4].get_center()+RIGHT*0.2)
		self.side_lengths = VGroup(
			MathTex("a", color=self.colors[0]).move_to(self.triangle[0].get_center()+UP*0.3+LEFT*0.3),
			MathTex("b", color=self.colors[1]).move_to(self.triangle[1].get_center()+UP*0.3+RIGHT*0.3),
			self.height
		)
		...
		self.play(
			Write(self.side_lengths)
		)
		self.wait(1)

	def build_tilted_rect(self):
		# Get the points of a rectangle with the same sidelengths
		# as the legs of the triangle and with the sides lying on
		...
		# The VGroup of lines representing the rectangle
	...
	def slice_and_rearrange_rect(self):
		...
		sect_lines = VGroup(
			# Line(points["C"], points["A"], color=self.colors[2]),
			Line(points["D"], points["hD"], color=self.colors[3])
		)
	...
	def show_rect_height_and_hypotenuse(self):
		self.hypotenuse = MathTex("c", color=self.colors[2]).move_to(self.triangle[2].get_center()+DOWN*0.5)
	...
	def separate_triangle_and_rect(self):
		self.play(
			self.triangle.animate.shift(UP).scale(0.75),
			self.rect.animate.shift(DOWN*3).scale(0.75)
		)
	...
	def respawn_tilted_rect(self):
		self.tilted_rect += self.tilted_rect_lines
		self.tilted_rect.move_to(self.triangle[2]).scale(0.75)
	...
	def write_rect_area(self):
		self.rect_area = MathTex("h","c").move_to(self.rect[0])
		...
		# self.remove(*self.rect[1:3])
		# self.rect.remove(*self.rect[1:3])
		self.remove(*self.rect_area[:2])
		self.add(self.rect_area)

	def write_tilted_rect_area(self):
		self.tilted_rect_area = MathTex("a","b").move_to(self.tilted_rect[0])
		...
		# self.remove(self.tilted_rect[1])
		# self.tilted_rect.remove(self.tilted_rect[1])
	...
	def do_math(self):
		...
		initial_formula = MathTex("h","c","=","a","b").move_to(pos)
		...
		self.final_formula = MathTex("h", "=", "{a", "b", "\\over \\,", "c}").move_to(pos)
		self._colorize_tex(self.final_formula)
		...
		self.play(
			*[
				ReplacementTransform(
					initial_formula.get_part_by_tex(tex),
					self.final_formula.get_part_by_tex(tex),
					run_time=1
				)
				for tex in ("h", "c", "=", "a", "b")
			],
			FadeIn(self.final_formula.get_part_by_tex("over"), run_time=1.5)
		)

		self.wait(3)

		self.play(
			self.triangle.animate.scale(1.5).shift(DOWN),
			self.final_formula.animate.scale(1.5).shift(DOWN)
		)
	...
	def fade_out(self):
		self.play(
			FadeOut(self.triangle, run_time=1),
			FadeOut(self.final_formula, run_time=1)
		)
